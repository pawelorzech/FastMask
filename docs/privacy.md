...
Crash reporting arrived in **FastMask 1.10.0**; version 1.9.0 and every version before it contain no crash‑reporting SDK and send nothing to Google. It is **on by default** and you can switch it off at any time in **Settings → Crash reports**, which stops collection immediately and deletes reports still queued on your device.

A crash report never contains your masked addresses, mask descriptions, associated domains, Fastmail API token, email address, or message content: the app's only calls to the Crashlytics SDK are "turn collection on/off" and "delete unsent reports", so there is no code path that could attach them. Debug builds never report at all. Firebase's session‑reporting SDK, which would otherwise ping Google every time you open the app, is switched off in the app manifest.
...
**Change on 2026-07-26:** crash reporting via Firebase Crashlytics was added, on by default and switchable off in Settings. It applies **from FastMask 1.10.0 onwards** — if you are still on 1.9.0 or earlier, that version contains no crash‑reporting SDK and the previous wording ("no crash reports, no Crashlytics") still describes what is installed on your device.
