...
| **Crash diagnostics** — stack trace, device model, Android version, app version, and the identifiers Firebase generates for the installation (Firebase installation ID and Crashlytics installation UUID) | Sent to **Firebase Crashlytics**, a Google service, when the app crashes; while reporting is on, the SDK also registers the installation identifier with Google so a crash can be tied to an install. Your on/off choice is stored locally via Android `DataStore` | To find and fix the bugs that make the app crash |

Crash reporting arrives in **FastMask 1.10.0**, the next release; version 1.9.0 and every version before it — including whatever is installed on your device today — contain no crash‑reporting SDK and send nothing to Google. It is **on by default** and you can switch it off at any time in **Settings → Crash reports**, which stops collection immediately and deletes reports still queued on your device.

A crash report never contains your masked addresses, mask descriptions, associated domains, Fastmail API token, email address, or message content: the app's only calls to the Crashlytics SDK are "turn collection on/off" and "delete unsent reports", so there is no code path that could attach them. The report itself is only sent when the app actually crashes, but while reporting is on the SDK does two things at app start without any crash involved — it registers the installation with Google (Firebase installation ID) and downloads its own configuration from `firebase-settings.crashlytics.com`, a request that carries the installation ID, your device model, Android version and the app's Firebase ID; switching the toggle off stops the configuration download, after which the only network traffic the app makes is to Fastmail, except that a Firebase installation ID which never finished registering may still be registered once. Debug builds never report at all. Firebase's session‑reporting SDK, which would otherwise ping Google every time you open the app, is switched off in the app manifest.
...
**Change on 2026-07-26:** crash reporting via Firebase Crashlytics is being added, on by default and switchable off in Settings. It applies **from FastMask 1.10.0 onwards**, which has not been released yet — until you update to it, the version on your device contains no crash‑reporting SDK and the previous wording ("no crash reports, no Crashlytics") still describes what is installed.
