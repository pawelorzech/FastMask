...
- **No Tracking**: No analytics, no advertising ID, no event or screen tracking, no profiling — Google Analytics is deliberately not part of the app
- **Opt-out Crash Reporting**: Release builds send crash diagnostics (stack trace, device model, Android version, app version) to Firebase Crashlytics so bugs can be fixed. It is **on by default** and can be turned off in Settings → *Crash reports*, which stops collection immediately and deletes reports still queued on the device. Reports never contain masked addresses, descriptions, associated domains, the Fastmail API token, your email address, or message content — the SDK is reachable from a single file (`FirebaseCrashlyticsReporter.kt`) that exposes nothing but the on/off switch, and `CrashReportingPrivacyTest` fails the build if another file touches it. Debug builds never report. See the [Privacy Policy](https://pawelorzech.github.io/FastMask/privacy.html).
