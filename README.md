...
- **Opt-out Crash Reporting** (from 1.10.0): release builds send crash diagnostics — stack trace, device model, Android and app version — to Firebase Crashlytics. On by default, off in Settings → *Crash reports*. Reports never carry masked addresses, descriptions, domains, your API token or email; the SDK is called from one file and `CrashReportingPrivacyTest` fails the build if that changes. While reporting is on the SDK also registers the install and fetches its config at startup; the toggle stops both. Debug builds never report. Details in the [Privacy Policy](https://pawelorzech.github.io/FastMask/privacy.html).
