...
- **Opt-out Crash Reporting** (from 1.10.0, the next release): Release builds send crash diagnostics (stack trace, device model, Android version, app version) to Firebase Crashlytics, **on by default**, switchable off in Settings → *Crash reports*. Reports never contain masked addresses, descriptions, associated domains, the Fastmail API token, your email address, or message content: the SDK is named in a single file (`FirebaseCrashlyticsReporter.kt`) that exposes nothing but the on/off switch, and `CrashReportingPrivacyTest` fails the build if another file touches it. A report itself only leaves the device on a real crash — but while reporting is on, the SDK also registers the install with Google and fetches its own configuration at app start, which sends the installation ID, device model and Android version without any crash involved. Switch the toggle off and both stop. Debug builds never report. See the [Privacy Policy](https://pawelorzech.github.io/FastMask/privacy.html).
