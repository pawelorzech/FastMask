...
- [ ] ISC-3: `./gradlew lint` kończy się sukcesem, raport przeczytany (probe: Read lint-results)
...
- [ ] ISC-5: Baseline zapisany: co się buduje, co nie, ostrzeżenia (probe: Read raportu)
...
- [ ] ISC-7: Emulator uruchomiony, apka zainstalowana i odpalona (probe: adb shell + screenshot)
...
- [ ] ISC-18: HTTP 401/403 → wykrywalny stan "token nieważny", nie generyczny crash (probe: Read + test)
- [ ] ISC-19: Timeout / brak sieci / DNS → czytelny komunikat błędu, nie crash (probe: Read + manual)
...
- [ ] ISC-24: Równoległe/wielokrotne kliknięcia (create, toggle, archive) nie duplikują żądań (probe: Read ViewModels)
- [ ] ISC-25: Coroutine cancellation — opuszczenie ekranu anuluje w toku; brak leaków (probe: Read)
- [ ] ISC-26: Rotacja ekranu nie gubi stanu (query, filtr, formularz create) (probe: manual)
- [ ] ISC-27: Process death — odtworzenie bez crasha (probe: manual adb kill)
- [x] ISC-28: Operacje sieciowe poza main thread (probe: Read — suspend/Dispatchers)
- [ ] ISC-29: StateFlow/SharedFlow użyte poprawnie (eventy nie giną przy rotacji) (probe: Read)
- [ ] ISC-30: Wygaśnięcie/cofnięcie tokenu w trakcie sesji → sensowna ścieżka (komunikat/redirect), nie pętla błędów (probe: Read + manual)
...
- [ ] ISC-43: security-crypto alpha — ryzyko ocenione, decyzja udokumentowana (bez zmiany bez migracji) (probe: Read Decisions)
...
- [ ] ISC-47: Schowek po kopiowaniu adresu: brak wrażliwych flag; adres maski to nie sekret, ale ocena zachowania (probe: Read + manual)
...
- [ ] ISC-50: ProGuard rules nie osłabiają bezpieczeństwa (brak -dontobfuscate globalnego, keep tylko konieczne) (probe: Read proguard-rules.pro)
...
- [ ] ISC-54: Dokument prywatności zgodny z faktycznym zachowaniem apki (probe: Read docs/privacy.md vs kod)
- [ ] ISC-55: SECURITY.md zgodny z implementacją (probe: Read + diff)
...
- [ ] ISC-58: Eventy jednorazowe (nawigacja, logout) nie giną i nie duplikują się (probe: Read)
- [ ] ISC-59: Klucze list w LazyColumn stabilne (id maski) (probe: Read ListScreen)
- [ ] ISC-60: remember/LaunchedEffect/DisposableEffect użyte poprawnie — brak side effects w kompozycji (probe: Read)
- [ ] ISC-61: Duplikacja kodu oceniona; oczywiste duplikaty zredukowane lub uzasadnione (probe: Read)
- [ ] ISC-62: Za duże composable/ViewModele zidentyfikowane (probe: wc -l + Read)
...
- [ ] ISC-69: Recompozycje: brak oczywistych niestabilnych parametrów na hot path listy (probe: Read)
- [ ] ISC-70: Anti: nie dodano zbędnej abstrakcji (nowe interfejsy tylko gdy rozwiązują problem) (probe: git diff review)

### E. Wydajność (ISC-71…78)
- [ ] ISC-71: Formatowanie dat/relative time nie wykonuje się na każdej rekompozycji od zera (probe: Read RelativeTime + ListScreen)
- [ ] ISC-72: Lista 500+ masek płynna — brak pracy O(n) w item lambda (probe: Read + demo z dużą listą)
- [ ] ISC-73: Start apki: brak ciężkiej pracy w Application.onCreate (probe: Read)
- [ ] ISC-74: Brak zbędnych żądań sieciowych (np. podwójny fetch przy starcie listy) (probe: Read + logi debug)
- [ ] ISC-75: Rozmiar release APK zanotowany; R8 + shrinkResources aktywne (probe: Bash ls + Read gradle)
- [ ] ISC-76: Fonty Google Fonts — zachowanie offline ocenione (fallback bez crasha) (probe: Read Type.kt + manual airplane)
- [ ] ISC-77: Obrazy/ikony bez nadmiarowych rozmiarów (probe: du -sh res)
- [ ] ISC-78: Anti: żadna "optymalizacja" bez dowodu problemu (probe: Decisions)

### F. Dostępność i i18n (ISC-79…92)
- [ ] ISC-79: Wszystkie ikony akcji mają contentDescription (probe: Grep contentDescription=null vs stringResource)
- [ ] ISC-80: Touch targets ≥48dp dla akcji na kartach (probe: Read + manual)
- [ ] ISC-81: Font scale 200% — ekrany używalne, bez ucięć krytycznych (probe: manual/screenshot)
- [ ] ISC-82: Kontrast warm-ink palety spełnia WCAG AA dla tekstu głównego (probe: obliczenie kontrastu z Color.kt)
- [ ] ISC-83: TalkBack — krytyczne ścieżki (login, lista, kopiowanie) ogłaszane sensownie (probe: Read semantics + manual)
- [ ] ISC-84: RTL (arabski) — layout nie łamie się (probe: manual screenshot)
...
- [ ] ISC-86: Brakujące tłumaczenia → fallback na EN bez crasha (probe: Read + manual)
- [x] ISC-87: Hardcoded strings w composable — wylistowane (probe: Grep)
- [ ] ISC-88: Plurale użyte poprawnie tam, gdzie liczby (probe: Grep plurals)
- [ ] ISC-89: Formaty dat zależne od locale (probe: Read RelativeTime)
- [x] ISC-90: locales_config.xml zgodny z faktycznymi katalogami values-* (probe: diff)
- [ ] ISC-91: Języki z długimi tekstami (DE) nie łamią przycisków (probe: manual/screenshot)
- [ ] ISC-92: Komunikaty błędów przetłumaczone, nie surowe wyjątki (probe: Read + Grep)

### G. Android / release (ISC-93…102)
- [ ] ISC-93: targetSdk 35 spełnia wymóg Play 2026 (probe: Read + web docs)
- [ ] ISC-94: Edge-to-edge poprawny na API 35 (probe: manual na emulatorze API 35)
- [ ] ISC-95: Predictive back — ocenione (enableOnBackInvokedCallback) (probe: Read manifest)
- [ ] ISC-96: Adaptive icon poprawny (foreground/background/monochrome) (probe: Read mipmap + emulator)
- [ ] ISC-97: Dark mode — wszystkie ekrany poprawne (probe: manual screenshots)
- [ ] ISC-98: App działa na API 26 (emulator lub uzasadniony skip z ryzykiem) (probe: manual/deferred)
...
- [ ] ISC-101: README/Play copy/privacy zgodne z aplikacją (probe: Read diff)
...
- [ ] ISC-104: Dobrze potwierdzone P2 naprawione lub świadomie odroczone z uzasadnieniem (probe: raport)
...
- [ ] ISC-111: `./gradlew lint` bez nowych błędów po zmianach (probe: Bash)
- [x] ISC-112: Anti: żaden test nie jest pusty/tautologiczny (probe: Read testów)

### I. Testy manualne (ISC-113…122)
- [ ] ISC-113: Pierwsze uruchomienie + welcome + demo mode (probe: screenshot)
- [ ] ISC-114: Login nieprawidłowym tokenem → czytelny błąd (probe: screenshot)
- [ ] ISC-115: Brak internetu → czytelny błąd (probe: screenshot airplane)
- [ ] ISC-116: Wyszukiwanie + wszystkie filtry działają (probe: screenshot)
- [ ] ISC-117: Kopiowanie adresu → feedback + zawartość schowka (probe: adb clipboard)
- [ ] ISC-118: Tworzenie maski (demo) end-to-end (probe: screenshot)
- [ ] ISC-119: Edycja, enable/disable, archiwizacja, przywrócenie (demo) (probe: screenshots)
- [ ] ISC-120: Logout → powrót do loginu, token wyczyszczony (probe: manual + Read prefs)
- [ ] ISC-121: Rotacja + process death na liście i formularzu (probe: manual)
- [ ] ISC-122: Jasny/ciemny motyw + font 200% + RTL screenshots (probe: screenshots)

### J. Audyt UX i raport (ISC-123…136)
- [ ] ISC-123: 12 przepływów użytkownika ocenione (kroki, feedback, cofnięcie, błędy) (probe: raport sekcja 7)
- [ ] ISC-124: Onboarding tokenu oceniony jako bariera #1 — konkretna analiza (probe: raport)
- [ ] ISC-125: Backlog A (quick wins) ≥5 pozycji w pełnym formacie (probe: raport sekcja 8)
- [ ] ISC-126: Backlog B (średnie) ≥4 pozycje (probe: raport)
- [ ] ISC-127: Backlog C (duże, bez implementacji) ≥4 pozycje (probe: raport)
- [ ] ISC-128: Każda propozycja: problem, dowód, rozwiązanie, efekt, nakład XS-XL, priorytet, pomiar privacy-first (probe: Read)
- [ ] ISC-129: Raport: executive summary + baseline + tabela problemów + zmiany + wyniki testów (probe: Read)
- [ ] ISC-130: Raport: problemy nienaprawione z ryzykiem i next steps (probe: Read)
- [ ] ISC-131: Raport: plan dzień/tydzień/2-4 tygodnie (probe: Read)
- [ ] ISC-132: Raport: oceny 1-10 z uzasadnieniami (8 wymiarów) (probe: Read)
- [ ] ISC-133: Wszystkie liczby w raporcie z faktycznych komend (nie "testy przechodzą") (probe: Read)
- [ ] ISC-134: Anti: żadna duża funkcja nie zaimplementowana bez zgody (probe: git diff)
- [ ] ISC-135: Anti: żaden test/lint/minifikacja nie wyłączone (probe: git diff)
...
- (brak wpisów — sekcja wypełniana w LEARN po refutacjach)
